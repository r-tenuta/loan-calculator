...
function setupIntialValues() {
  const values = { amount: 500000, years: 30, rate: 5.5 };
...
function update() {
  ...
  return updateMonthly(calculateMonthlyPayment(current));
}
...
function calculateMonthlyPayment(values) {
  ...
  let payment = values.amount * interest / (1 - (Math.pow(1 + interest, -n)));
...
function updateMonthly(monthly) {
  ...
  return monthlyValue.innerText = "$" + monthly;
}
